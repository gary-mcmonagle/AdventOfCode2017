...
class bridge_maker:
    ...
    def generate_bridges(self, available_components, required_node, current_size):
        for idx, com in enumerate(available_components):
            if(not com.does_match(required_node) == False):
                new_list = available_components.copy()
                new_list.pop(idx)
                self.generate_bridges(new_list, com.does_match(required_node), current_size+com.get_value())
