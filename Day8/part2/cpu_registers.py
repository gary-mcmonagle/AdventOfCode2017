...
class cpu_registers:
    def __init__(self):
        ...
        self.largest_registery_value = 0
    ...
    def get_largest_registery_value(self):
        return self.largest_registery_value
    ...
    def execute_instruction(self, instruction):
        if(self.evaluate_condition(instruction.get_condition())):
            ...
            if(instruction.get_direction() == "dec"):
                self.find_register(instruction.get_register()).set_value(self.find_register(instruction.get_register()).get_value() - instruction.get_amount())
        if (self.get_largest_registery() > self.largest_registery_value):
            self.largest_registery_value = self.get_largest_registery()
